...
import { pushSlot, renderTable, logAppend, getScheduleStats, renderStats, exportTasksAsJSON, downloadJSON, presets } from './common.js';
...
// sample tasks
tasks = [
  { id:0, name:'A', arrival:0, exec:80, remaining:80, deadline:300, finished:false, missed:false, color:'color0' },
  { id:1, name:'B', arrival:30, exec:120, remaining:120, deadline:350, finished:false, missed:false, color:'color1' },
  { id:2, name:'C', arrival:80, exec:60, remaining:60, deadline:260, finished:false, missed:false, color:'color2' },
];
...
updateTaskTable();
logAppend(logOut, '🟢 EDF Scheduler Ready. Add tasks or click Start to begin.');
...
function step(){
  ...
  logAppend(logOut, `t=${now} : running ${cur.name} for ${q}ms (deadline ${cur.deadline})`);
  ...
  if (isIO) {
    ...
    if (readyNext.length > 0) {
      readyNext.sort((a,b)=> a.deadline - b.deadline);
      ...
      logAppend(logOut, `t=${now} : running ${nextTask.name} for ${q2}ms (deadline ${nextTask.deadline})`);
...
function updateNextTask() {
  ...
  // Sort by deadline (EDF algorithm)
  ready.sort((a,b)=> a.deadline - b.deadline);
  const next = ready[0];
  nextTaskInfo.innerHTML = `🟢 ${next.name} (deadline: ${next.deadline}ms, remaining: ${next.remaining}ms)`;
}
...
form.addEventListener('submit', (e)=>{
  ...
  const t = {
    ...
    deadline: Number(data.get('deadline') || 200),
    ioOps: ioOps,
    ioTime: ioTime,
    ioCount: 0,
    suspended: false,
  ...
  logAppend(logOut, `✓ Task "${t.name}" added (arrival=${t.arrival}, exec=${t.exec}, deadline=${t.deadline}${ioInfo})`);
  form.reset();
});
...
document.getElementById('runEDF').addEventListener('click', ()=>{
  ...
  sim = setInterval(()=>{
    ...
    const delay = Number(execDelay.value) || 50;
...
document.getElementById('exportBtn').addEventListener('click', () => {
  ...
  downloadJSON(json, 'edf_tasks.json');
  logAppend(logOut, '✓ Tasks exported to edf_tasks.json');
});
